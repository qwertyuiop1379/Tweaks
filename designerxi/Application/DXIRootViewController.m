...
-(void)loadThemeObjects
{
	...
	for (NSString *line in [manager contentsOfDirectoryAtPath:[[self getCurrentProjectDirectory] stringByAppendingString:@"/Bundles"] error:nil])
	{
...
-(void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
	...
	if (tableView == self.tableView)
	{
		...
		if ([self getCurrentProject].type == 0)
		{
			...
			if ([manager fileExistsAtPath:[[self getCurrentProjectDirectory] stringByAppendingString:@"/icon.png"]])
			{
				iconPreview.image = [UIImage imageWithContentsOfFile:[[self getCurrentProjectDirectory] stringByAppendingString:@"/icon.png"]];
			}
			else
			{
	...
	else
	{
		...
		iconPreview.layer.cornerRadius = 10;
		iconPreview.layer.masksToBounds = YES;
		if ([manager fileExistsAtPath:[NSString stringWithFormat:@"%@/Bundles/%@@3x.png", [self getCurrentProjectDirectory], _themeObjects[indexPath.row]]])
		{
			iconPreview.image = [UIImage imageWithContentsOfFile:[NSString stringWithFormat:@"%@/Bundles/%@@3x.png", [self getCurrentProjectDirectory], _themeObjects[indexPath.row]]];
		}
		else
		{
...
-(void)backToThemeEditView
{
	...
	self.title = [NSString stringWithFormat:@"%@%@", [self getCurrentProject].name, [self getCurrentProject].type == 0 ? @" (Icon)":@" (Theme)"];
	self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(backToHome)];
	self.navigationItem.rightBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addThemeBundle)];
...
{
	NSArray *apps = [[NSClassFromString(@"LSApplicationWorkspace") defaultWorkspace] allInstalledApplications];
...
-(void)removeThemeIcon:(NSIndexPath *)indexPath
{
	...
	{
		NSString *path = [NSString stringWithFormat:@"%@/Bundles/%@@3x.png", [self getCurrentProjectDirectory], _themeObjects[indexPath.row]];
...
-(void)imagePickerController:(UIImagePickerController *)picker didFinishPickingMediaWithInfo:(NSDictionary *)info
{
	...
	if ([self getCurrentProject].type == 0)
	{
		...
		[UIImagePNGRepresentation(image) writeToFile:[[self getCurrentProjectDirectory] stringByAppendingString:@"/icon.png"] atomically:YES];
	}
	else
	{
		image = [image scaleToSize:CGSizeMake(180, 180)];
		[UIImagePNGRepresentation(image) writeToFile:[NSString stringWithFormat:@"%@/Bundles/%@@3x.png", [self getCurrentProjectDirectory], _themeObjects[openedBundleIndex.row]] atomically:YES];
	}
...
-(void)deleteProject:(NSIndexPath *)index
{
    ...
    [confirm addAction:[UIAlertAction actionWithTitle:@"Delete" style:UIAlertActionStyleDestructive handler:^(UIAlertAction *action)
	{
		NSString *projectDir = [self getCurrentProjectDirectory];
...
{
	return [NSString stringWithFormat:@"%@%d%@", @"/User/Documents/DesignerXI/", [self getCurrentProject].type, [self getCurrentProject].name];
}
...
-(void)optionsPressed
{
	...
	[actionSheet addAction:[UIAlertAction actionWithTitle:@"Build Project" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
	{
		[self startAnimating];
		[self performSelectorInBackground:@selector(buildCurrentProject) withObject:nil];
	}]];
	[actionSheet addAction:[UIAlertAction actionWithTitle:@"Install Project" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action)
	{
		[self startAnimating];
		[self performSelectorInBackground:@selector(installCurrentProject) withObject:nil];
	}]];
	[actionSheet addAction:[UIAlertAction actionWithTitle:@"Edit Control File" style:UIAlertActionStyleDefault handler:^(UIAlertAction *action){ [self openEditorForControl]; }]];
...
-(void)doTheThing:(BOOL)install
{
	[manager removeItemAtPath:@"/User/Documents/DesignerXI/tmp" error:nil];
	[manager removeItemAtPath:@"/User/Documents/DesignerXI/package.deb" error:nil];
	if ([manager fileExistsAtPath:[[self getCurrentProjectDirectory] stringByAppendingString:@"/control"]])
	{
		...
		[manager copyItemAtPath:[[self getCurrentProjectDirectory] stringByAppendingString:@"/control"] toPath:@"/User/Documents/DesignerXI/tmp/DEBIAN/control" error:nil];
		if ([self getCurrentProject].type == 0)
		{
			if ([manager fileExistsAtPath:[[self getCurrentProjectDirectory] stringByAppendingString:@"/icon.png"]])
			{
				...
				UIImage *icon = [UIImage imageWithContentsOfFile:[[self getCurrentProjectDirectory] stringByAppendingString:@"/icon.png"]];
		...
		else
		{
			...
			for (NSString *line in items)
			{
				...
				[manager copyItemAtPath:[NSString stringWithFormat:@"%@/Bundles/%@", [self getCurrentProjectDirectory], line] toPath:toSave error:nil];
			}
		...
		_system("dpkg-deb -Zlzma -b /User/Documents/DesignerXI/tmp /User/Documents/DesignerXI/package.deb");
		if (install)
		{
			...
			{
				[self error:@"Error" message:lastOutput];
				return;
			}
		}
		[spinner stopAnimating];
...
-(void)openEditorForControl
{
	NSString *controlPath = [[self getCurrentProjectDirectory] stringByAppendingString:@"/control"];
	editorView = [[UIView alloc] initWithFrame:frame];
	editorView.backgroundColor = [UIColor blackColor];
	controlContents = [NSString stringWithContentsOfFile:controlPath encoding:NSUTF8StringEncoding error:nil];
	self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(closeEditorView)];
	self.navigationItem.rightBarButtonItem = nil;
	[self.view addSubview:editorView];
	textView = [[UITextView alloc] initWithFrame:CGRectMake(10, 10, frame.size.width - 20, 300)];
	textView.text = controlContents;
	...
	[editorView setAlpha:0.0f];
	[UIView animateWithDuration:0.3f animations:^{ [editorView setAlpha:1.0f]; } completion:nil];
}

-(void)closeEditorView
{
	self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemDone target:self action:@selector(backToHome)];
	self.navigationItem.rightBarButtonItem = [self getCurrentProject].type == 1 ? [[UIBarButtonItem alloc] initWithBarButtonSystemItem:UIBarButtonSystemItemAdd target:self action:@selector(addThemeBundle)] : nil;
	controlContents = textView.text;
	[controlContents writeToFile:[[self getCurrentProjectDirectory] stringByAppendingString:@"/control"] atomically:YES encoding:NSStringEncodingConversionAllowLossy error:nil];
	[editorView setAlpha:1.0f];
	[UIView animateWithDuration:0.3f animations:^{ [editorView setAlpha:0.0f]; } completion:^(BOOL finished){ [editorView removeFromSuperview]; }];
}
